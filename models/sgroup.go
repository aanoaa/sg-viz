...
var SgroupWhere = struct {
	...
	ID:   whereHelpernull_Int64{field: "\"sgroup\".\"id\""},
...
var SgroupRels = struct {
	...
	FromPolicies: "FromPolicies",
}

// sgroupR is where relationships are stored.
type sgroupR struct {
	Hosts        HostSlice   `boil:"Hosts" json:"Hosts" toml:"Hosts" yaml:"Hosts"`
	ToPolicies   PolicySlice `boil:"ToPolicies" json:"ToPolicies" toml:"ToPolicies" yaml:"ToPolicies"`
	FromPolicies PolicySlice `boil:"FromPolicies" json:"FromPolicies" toml:"FromPolicies" yaml:"FromPolicies"`
}
...
func (r *sgroupR) GetHosts() HostSlice {
	if r == nil {
		return nil
	}
	return r.Hosts
...
func (r *sgroupR) GetToPolicies() PolicySlice {
	if r == nil {
		return nil
	}
	return r.ToPolicies
...
func (r *sgroupR) GetFromPolicies() PolicySlice {
	if r == nil {
		return nil
	}
	return r.FromPolicies
}
...
func (o *Sgroup) Hosts(mods ...qm.QueryMod) hostQuery {
	...
	queryMods = append(queryMods,
		qm.InnerJoin("\"host_sgroup\" on \"host\".\"id\" = \"host_sgroup\".\"host_id\""),
		qm.Where("\"host_sgroup\".\"sgroup_id\"=?", o.ID),
	)

	return Hosts(queryMods...)
}

// ToPolicies retrieves all the policy's Policies with an executor via to column.
func (o *Sgroup) ToPolicies(mods ...qm.QueryMod) policyQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"policy\".\"to\"=?", o.ID),
	)

	return Policies(queryMods...)
}

// FromPolicies retrieves all the policy's Policies with an executor via from column.
func (o *Sgroup) FromPolicies(mods ...qm.QueryMod) policyQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
...
func (sgroupL) LoadHosts(ctx context.Context, e boil.ContextExecutor, singular bool, maybeSgroup interface{}, mods queries.Applicator) error {
	var slice []*Sgroup
	var object *Sgroup

	if singular {
		var ok bool
		object, ok = maybeSgroup.(*Sgroup)
		if !ok {
			object = new(Sgroup)
			ok = queries.SetFromEmbeddedStruct(&object, &maybeSgroup)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", object, maybeSgroup))
			}
		}
	} else {
		s, ok := maybeSgroup.(*[]*Sgroup)
		if ok {
			slice = *s
		} else {
			ok = queries.SetFromEmbeddedStruct(&slice, maybeSgroup)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", slice, maybeSgroup))
			}
		}
	}
	...
	query := NewQuery(
		qm.Select("\"host\".\"id\", \"host\".\"hostname\", \"host\".\"ipaddr\", \"host\".\"desc\", \"a\".\"sgroup_id\""),
		qm.From("\"host\""),
		qm.InnerJoin("\"host_sgroup\" as \"a\" on \"host\".\"id\" = \"a\".\"host_id\""),
		qm.WhereIn("\"a\".\"sgroup_id\" in ?", args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	for results.Next() {
		...
		err = results.Scan(&one.ID, &one.Hostname, &one.Ipaddr, &one.Desc, &localJoinCol)
		if err != nil {
			return errors.Wrap(err, "failed to scan eager loaded results for host")
		}
		if err = results.Err(); err != nil {
			return errors.Wrap(err, "failed to plebian-bind eager loaded slice host")
		}
	...
	if singular {
		...
		for _, foreign := range resultSlice {
			...
			foreign.R.Sgroups = append(foreign.R.Sgroups, object)
		}
		return nil
	}
	...
	for i, foreign := range resultSlice {
		...
		for _, local := range slice {
			if queries.Equal(local.ID, localJoinCol) {
				...
				foreign.R.Sgroups = append(foreign.R.Sgroups, local)
				break
			}
		}
	}
...
// LoadToPolicies allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (sgroupL) LoadToPolicies(ctx context.Context, e boil.ContextExecutor, singular bool, maybeSgroup interface{}, mods queries.Applicator) error {
	var slice []*Sgroup
	var object *Sgroup
	...
	if singular {
		object.R.ToPolicies = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &policyR{}
			}
			foreign.R.ToSgroup = object
		}
		return nil
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if queries.Equal(local.ID, foreign.To) {
				local.R.ToPolicies = append(local.R.ToPolicies, foreign)
				if foreign.R == nil {
					foreign.R = &policyR{}
				}
				foreign.R.ToSgroup = local
				break
			}
		}
	}
...
// LoadFromPolicies allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (sgroupL) LoadFromPolicies(ctx context.Context, e boil.ContextExecutor, singular bool, maybeSgroup interface{}, mods queries.Applicator) error {
	var slice []*Sgroup
	var object *Sgroup
	...
	query := NewQuery(
		qm.From(`policy`),
		qm.WhereIn(`policy.from in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.FromPolicies = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &policyR{}
			}
			foreign.R.FromSgroup = object
		}
		return nil
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if queries.Equal(local.ID, foreign.From) {
				local.R.FromPolicies = append(local.R.FromPolicies, foreign)
				if foreign.R == nil {
					foreign.R = &policyR{}
				}
				foreign.R.FromSgroup = local
				break
			}
		}
	}
...
// of the sgroup, optionally inserting them as new records.
// Appends related to o.R.Hosts.
// Sets related.R.Sgroups appropriately.
func (o *Sgroup) AddHosts(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Host) error {
	var err error
	...
	for _, rel := range related {
		query := "insert into \"host_sgroup\" (\"sgroup_id\", \"host_id\") values (?, ?)"
	...
	for _, rel := range related {
		if rel.R == nil {
			rel.R = &hostR{
				Sgroups: SgroupSlice{o},
			}
		} else {
			rel.R.Sgroups = append(rel.R.Sgroups, o)
		}
	}
	return nil
}
...
// sgroup replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Sgroups's Hosts accordingly.
// Replaces o.R.Hosts with related.
// Sets related.R.Sgroups's Hosts accordingly.
func (o *Sgroup) SetHosts(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Host) error {
	query := "delete from \"host_sgroup\" where \"sgroup_id\" = ?"
	...
	removeHostsFromSgroupsSlice(o, related)
	if o.R != nil {
		o.R.Hosts = nil
	}
...
// Sets related.R.Sgroups.
func (o *Sgroup) RemoveHosts(ctx context.Context, exec boil.ContextExecutor, related ...*Host) error {
	if len(related) == 0 {
		return nil
	}
	...
	var err error
	query := fmt.Sprintf(
		"delete from \"host_sgroup\" where \"sgroup_id\" = ? and \"host_id\" in (%s)",
...
func removeHostsFromSgroupsSlice(o *Sgroup, related []*Host) {
	for _, rel := range related {
		if rel.R == nil {
			continue
		}
		for i, ri := range rel.R.Sgroups {
			if !queries.Equal(o.ID, ri.ID) {
				continue
			}

			ln := len(rel.R.Sgroups)
			if ln > 1 && i < ln-1 {
				rel.R.Sgroups[i] = rel.R.Sgroups[ln-1]
			}
			rel.R.Sgroups = rel.R.Sgroups[:ln-1]
			break
		}
	}
}

// AddToPolicies adds the given related objects to the existing relationships
// of the sgroup, optionally inserting them as new records.
// Appends related to o.R.ToPolicies.
// Sets related.R.ToSgroup appropriately.
func (o *Sgroup) AddToPolicies(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Policy) error {
	var err error
	for _, rel := range related {
		if insert {
			queries.Assign(&rel.To, o.ID)
			if err = rel.Insert(ctx, exec, boil.Infer()); err != nil {
				return errors.Wrap(err, "failed to insert into foreign table")
			}
		} else {
	...
	if o.R == nil {
		...
		o.R.ToPolicies = append(o.R.ToPolicies, related...)
	}
...
// AddFromPolicies adds the given related objects to the existing relationships
// of the sgroup, optionally inserting them as new records.
// Appends related to o.R.FromPolicies.
// Sets related.R.FromSgroup appropriately.
func (o *Sgroup) AddFromPolicies(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Policy) error {
	var err error
	for _, rel := range related {
		if insert {
			queries.Assign(&rel.From, o.ID)
			if err = rel.Insert(ctx, exec, boil.Infer()); err != nil {
				return errors.Wrap(err, "failed to insert into foreign table")
			}
		} else {
	...
	if o.R == nil {
		o.R = &sgroupR{
			FromPolicies: related,
		}
	} else {
		o.R.FromPolicies = append(o.R.FromPolicies, related...)
	}
...
func FindSgroup(ctx context.Context, exec boil.ContextExecutor, iD null.Int64, selectCols ...string) (*Sgroup, error) {
	sgroupObj := &Sgroup{}
...
func SgroupExists(ctx context.Context, exec boil.ContextExecutor, iD null.Int64) (bool, error) {
	var exists bool
